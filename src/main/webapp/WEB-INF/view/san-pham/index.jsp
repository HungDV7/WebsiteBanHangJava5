...
<div class="container">
    <h4>
        <center>DANH SÁCH ĐỒNG HỒ</center>
    </h4>
    <hr>
    <div class="row">
        <div class="col-6">
            <div style="margin-bottom: 20px">
                <a href="/admin/dong-ho/create" class="btn btn-outline-primary">Thêm mới</a>
            </div>
        </div>
        <div class="col-6">
            <div style="margin-bottom: 20px">
                <a href="/shop-dong-ho" class="btn btn-outline-danger">Về trang chủ</a>
            </div>
        </div>
    </div>
        ...
        <table class="table">
            ...
            <c:forEach items="${ds.getContent()}" var="sp" varStatus="a">
                <tr>
                    ...
                    <td>${sp.soLuongTon}</td>
                    <td>${sp.donGia}</td>
                    <td><a class="btn btn-info" onclick="return confirm('Bạn có muốn xem chi tiết ?')"
                           href="/admin/dong-ho/detail/${sp.id}">Detail</a></td>
                    <td><a class="btn btn-info" onclick="return confirm('Bạn chắc chắn xóa chứ ?')"
                           href="/admin/dong-ho/delete/${sp.id}">Delete</a></td>
                </tr>
            </c:forEach>
            </tbody>
        </table>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <c:if test="${ds.getNumber() gt 0}">
                            <a href="?tenSanPham=${param.tenSanPham}&min=${param.min}&max=${param.max}&page=${ds.getNumber()}"
                    ...
                    <div class="col-md-4">
                        <c:if test="${ds.getNumber() lt ds.getTotalPages()-1}">
                            <a href="?tenSanPham=${param.tenSanPham}&min=${param.min}&max=${param.max}&page=${ds.getNumber()+2}"
